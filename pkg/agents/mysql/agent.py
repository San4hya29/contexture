"""
Natural-language query router for the MySQL agent.

Maps simple keyword patterns to tool calls — mirrors clickhouse/agent.py.
...
def process_query(query: str) -> dict:
    ...
    if ("stat" in q or "size" in q or "row" in q) and "table" in q:
        ...
        return {"message": "Please specify table name, e.g. 'stats for table test_db.orders'"}
    ...
    if "describe" in q or "columns" in q or "schema of" in q:
        ...
        return {"message": "Please specify table, e.g. 'describe test_db.orders'"}
