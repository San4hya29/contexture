"""
MCP tool wrapper functions for the MySQL agent.
...
def list_tables_tool(database: str = "") -> Dict[str, Any]:
    results = {}
    for inst in _instances():
        try:
    ...
    for inst in _instances():
        try:
            ...
            results[inst["name"]] = stats if stats else {
                "error": f"Table {database}.{table} not found in information_schema"
            }
        except Exception as e:
            results[inst["name"]] = {"error": str(e)}
    return results
...
def get_slow_queries_tool(min_duration_sec: float = 1.0, limit: int = 10) -> Dict[str, Any]:
    results = {}
    for inst in _instances():
        try:
            results[inst["name"]] = get_slow_queries(inst, min_duration_sec, limit)
        except Exception as e:
            results[inst["name"]] = {"error": str(e)}
    return results
