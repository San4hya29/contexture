#!/usr/bin/env python3
"""
MySQL NL Chatbot — Terminal-based natural language query interface.

Routes ALL queries through the MySQL MCP Server (port 8005)
using FastMCP Client. No direct MySQL connection.

Usage:
    1. Start the MCP server:  cd pkg/agents/mysql && python server.py --transport sse --port 8005
    2. Run chatbot:           source .venv/bin/activate && python scripts/mysql/chatbot.py
"""
...
# ── MCP Client (connects to MySQL MCP Server) ───────────────────────────
MCP_URL = server_cfg.get("mysql_mcp_url", "http://localhost:8005/sse")
...
def nl_to_sql(question):
    ...
    prompt = (
        "You are a MySQL SQL expert. Given the database schema below, "
        ...
        "- Use MySQL-compatible syntax (COUNT(*), etc.).\n"
...
async def mcp_execute_query(session, sql):
    """Execute SQL via my_execute_query MCP tool. Returns (rows, columns)."""
    result = await session.call_tool("my_execute_query", {"sql": sql, "limit": 200})
    ...
    for inst_name, inst_data in instances.items():
        if "error" in inst_data:
            raise RuntimeError(f"MySQL error ({inst_name}): {inst_data['error']}")
...
async def mcp_health_check(session):
    """Check MySQL health via my_check_db_health MCP tool."""
    result = await session.call_tool("my_check_db_health", {})
    parsed = _parse_mcp_result(result)
...
async def main():
    ...
    print(DIM + "  Connecting to MySQL MCP Server..." + RESET)
        ...
        async with mcp_client as session:
            ...
            try:
                ...
                for inst_name, data in health.items():
                    ...
                    print(GREEN + f"  Connected via MCP → MySQL v{ver}" + RESET)
                ...
                print(DIM + "    cd pkg/agents/mysql && python server.py --transport sse --port 8005" + RESET + "\n")
                sys.exit(1)
...
""" + RESET)
        ...
        print(DIM + "\n  Make sure the MySQL MCP Server is running:" + RESET)
        print(BOLD + "    cd pkg/agents/mysql && python server.py --transport sse --port 8005" + RESET + "\n")
        sys.exit(1)
